<template>
	<div class="footerBox-new">
		<g-hq-mask v-if="showTrialCon" @onClick=""></g-hq-mask>
		<div class="footer">
			<div v-show="showTrialCon" class="trialCon">
				<popup-picker v-if="trialElementArr.includes(22)" ref="Plan" class="tri-vux-select" 
					:title="'可选方案'" 
					:data="listPlan" 
					v-model="valuePlan" 
					:placeholder="'请选择投保方案'" 
				...
				<g-hq-gender  v-if="trialElementArr.includes(22)" ref="Sex" :value="valueSex" :titleWidth="8.25" @onChange="valueSex = arguments[0]"></g-hq-gender>
				<group class="address-group" v-if="trialElementArr.includes(22)" ref="Address">
					<x-address   title="地址" v-model="valueAddress" :list="listAddress" placeholder="请选择投保地区" value-text-align="left"></x-address>
				</group>
				...
				<popup-picker v-if="trialElementArr.includes(22)" ref="Medicare" class="tri-vux-select" 
					:title="'医疗保险'" 
					:data="listMedicare" 
					v-model="valueMedicare" 
					:placeholder="'请选择是否有医疗保险'" 
					@on-change="onChange" 
					value-text-align="left" >
				</popup-picker>
				...
				<g-hq-input v-if="trialElementArr.includes(22)" ref="payWay" class="pl" title="交费方式"  :titleWidth="8.25" disabled="true"  :value="payWay"></g-hq-input>
				...
				<g-hq-input v-if="trialElementArr.includes(22)" ref="EachCoverage" class="pl" title="每份保额"  :titleWidth="8.25" disabled="true"  :value="valueEachCoverage"></g-hq-input>
				<group class="number-group" v-if="trialElementArr.includes(22)" ref="numberValue">
				    <x-number title="份数"  v-model="numberValue" :max="numberMax" :min="numberMix"></x-number>
				</group>
...
<script>
// 可选方案 出生日期 性别 地址 交费期间 医疗保险 保险期间 交费方式 保额 每份保额 份数
	import bottom from './bottom.vue'
	import { PopupPicker, Group , Datetime ,XNumber, XAddress,TransferDomDirective as TransferDom} from 'vux'
	let ChinaAddressV3Data = require("../../assets/js/ChinaCity.json")
	export default {
		name: "ProductTrialEase",
		props: {
  			trialRule:{
  				required:false
  			},
